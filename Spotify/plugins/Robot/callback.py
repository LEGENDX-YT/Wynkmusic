...
from Spotify.utils.formatters import seconds_to_min
from Spotify.utils.inline.play import (stream_markup,
                                          telegram_markup)
from Spotify.utils.stream.autoclear import auto_clean
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                await Spotify.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Spotify.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
